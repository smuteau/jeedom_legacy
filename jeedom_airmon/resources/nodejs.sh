#!/bin/bash
cd $1
touch /tmp/airmon_dep
echo "Début de l'installation"

echo 0 > /tmp/airmon_dep
DIRECTORY="/var/www"
if [ ! -d "$DIRECTORY" ]; then
...
echo 70 > /tmp/airmon_dep

sudo apt-get -y install aircrack-ng

echo 75 > /tmp/airmon_dep
...
echo 80 > /tmp/airmon_dep
npm install JSON2
echo 85 > /tmp/airmon_dep
npm install xml2json
echo 80 > /tmp/airmon_dep
npm install JSON2
echo 85 > /tmp/airmon_dep
npm install watch
echo 90 > /tmp/airmon_dep
