...
$("#bt_addxivoInfo").on('click', function(event) {
    var _cmd = {type: 'info'};
    addCmdToTable(_cmd);
});

$("#bt_addxivoAction").on('click', function(event) {
...
function addCmdToTable(_cmd) {
    ...
    if (init(_cmd.type) == 'action') {
        ...
        tr += '</div>';
        tr += '</div>';
        tr += '<select class="cmdAttr form-control tooltips input-sm" data-l1key="configuration" data-l2key="type" title="{{Est-ce une info en pull depuis Xivo ou pour commande}}">';
