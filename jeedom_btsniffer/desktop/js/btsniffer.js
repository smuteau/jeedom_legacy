...
$("#bt_addbtsnifferInfo").on('click', function(event) {
    var _cmd = {type: 'info'};
    addCmdToTable(_cmd);
});

$("#bt_addbtsnifferAction").on('click', function(event) {
...
$('body').on('btsniffer::includeDevice', function (_event,_options) {
    if (modifyWithoutSave) {
        ...
        if (_options == '') {
            ...
            window.location.href = 'index.php?v=d&p=btsniffer&m=btsniffer&id=' + _options;
        }
    }
});
...
function printModuleInfo(_id) {
    $.ajax({// fonction permettant de faire de l'ajax
        ...
        url: "plugins/btsniffer/core/ajax/btsniffer.ajax.php", // url du fichier php
        data: {
        ...
        success: function (data) { // si l'appel a bien fonctionné
            ...
            for (var i in data.result) {
                ...
                $('.btsnifferInfo[data-l1key=' + i + ']').value(value);
                $('.btsnifferInfo[data-l1key=' + i + ']').attr('title', data.result[i]['datetime']);
            }
        }
    });
}
