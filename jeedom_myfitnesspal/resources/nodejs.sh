#!/bin/bash
cd $1
touch /tmp/myfitnesspal_dep
echo "Début de l'installation"

echo 0 > /tmp/myfitnesspal_dep
DIRECTORY="/var/www"
if [ ! -d "$DIRECTORY" ]; then
...
echo 80 > /tmp/myfitnesspal_dep
npm install mfp
echo 85 > /tmp/myfitnesspal_dep
npm install request
echo 90 > /tmp/myfitnesspal_dep

sudo chown -R www-data *

rm /tmp/myfitnesspal_dep
