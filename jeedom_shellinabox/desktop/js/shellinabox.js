...
$("#bt_addshellinaboxInfo").on('click', function(event) {
    var _cmd = {type: 'info'};
    addCmdToTable(_cmd);
});

$("#bt_addshellinaboxAction").on('click', function(event) {
...
function printModuleInfo(_id) {
    $.ajax({// fonction permettant de faire de l'ajax
        ...
        url: "plugins/shellinabox/core/ajax/shellinabox.ajax.php", // url du fichier php
        data: {
        ...
        success: function (data) { // si l'appel a bien fonctionné
            ...
            $('.shellinaboxInfo').value('');
            for (var i in data.result) {
                var value = data.result[i]['value'];
                if (isset(data.result[i]['unite'])) {
                    value += ' ' + data.result[i]['unite'];
                }
                $('.shellinaboxInfo[data-l1key=' + i + ']').value(value);
                $('.shellinaboxInfo[data-l1key=' + i + ']').attr('title', data.result[i]['datetime']);
            }
        }
    });
}
